...
import { Engine, Scene, SceneLoader, Vector3, Mesh, MeshBuilder, FreeCamera, ArcRotateCamera, Color4, StandardMaterial, Color3, PointLight, HemisphericLight, ShadowGenerator, Quaternion, Matrix, VideoTexture, VideoDome, PointerEventTypes } from '@babylonjs/core';
...
class App {
	...
	async goToCutScene(){
		...
		let loadedCutGui = await cutScene.parseFromSnippetAsync('9HWE6N#14'); //MGGNCX#9
	...
	async loadCharacterAssets(scene){
		...
		async function loadCharacter(){
			...
			/*let essai = SceneLoader.ImportMesh('', '/Character.glb', '',scene, function (newMeshes) {
				let personnage = newMeshes[0];
			}); */
			
			let personnage = scene.getMeshByName('__root__');
	...
	async goToGame(){
		...
		//create a simple button
		const loseBtn = Button.CreateSimpleButton('lose', 'QUIT');
		loseBtn.width = 0.2;
		loseBtn.height = '40px';
		loseBtn.color = 'white';
		loseBtn.top = '-30px';
	...
	async goToVideo(){
		...
		const lectureBtn = Button.CreateImageOnlyButton('lecturebutton', '/play-pause-btn.png');
		lectureBtn.width = '40px';
	...
	async goToVideo360(){
		...
		//--SCENE SETUP--
		//this.#scene.detachControl();
		let dome, tickCount = -240, zoomLevel = 5;
		let scene = new Scene(this.#engine);
		let camera = new ArcRotateCamera('arcR', -Math.PI/2, Math.PI/2, 1200, new Vector3(0,0,0),scene);
		camera.attachControl(this.#canvas, true);
		dome = new VideoDome(
		...
		//--GUI--
		const guiMenu = AdvancedDynamicTexture.CreateFullscreenUI('UI');
		const mainBtn = Button.CreateSimpleButton('returnsite', 'RETURN SITE');
		mainBtn.width = 0.2;
		mainBtn.height = '40px';
		mainBtn.color = 'white';
		guiMenu.addControl(mainBtn);
